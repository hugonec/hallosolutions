  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-slate-50 text-slate-900 font-sans">
      <header className="bg-white shadow-md sticky top-0 z-10">
        <div className="max-w-6xl mx-auto p-6 flex flex-col sm:flex-row items-center justify-between">
          ...
          <div className="mt-4 sm:mt-0 space-x-4">
            <button className="bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-md">For Brands</button>
            <button className="border px-4 py-2 rounded-md">For 3PLs</button>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto p-8 space-y-24">
        ...
        >
          <h1 className="text-4xl sm:text-5xl font-bold mb-4">Find Your Ideal 3PL Partner in Europe</h1>
          <p className="text-xl text-slate-600 max-w-2xl mx-auto">Hallo Solutions connects growing brands with top-tier 3PLs — fast, free, and tailored to your needs.</p>
          <div className="mt-8 flex justify-center gap-4">
            <button className="bg-orange-500 hover:bg-orange-600 text-white px-6 py-3 text-lg rounded-md">Start as a Brand</button>
            <button className="border px-6 py-3 text-lg rounded-md">Register as a 3PL</button>
          </div>
        </motion.section>
        ...
        >
          ...
          <button className="bg-orange-500 hover:bg-orange-600 text-white px-6 py-3 rounded-md">Contact Hallo Solutions</button>
        </motion.section>
      </main>

      <footer className="bg-white border-t text-center text-sm text-slate-500 p-4">
        © {new Date().getFullYear()} Hallo Solutions. All rights reserved.
      </footer>
    </div>
  );
}
